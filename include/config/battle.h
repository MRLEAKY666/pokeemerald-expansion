...
#define B_MULTI_HIT_CHANCE          GEN_3 // In Gen5+, multi-hit moves have different %. See SetRandomMultiHitCounter for values.
...
#define B_LIGHT_BALL_ATTACK_BOOST   GEN_3 // In Gen4+, Light Ball doubles the power of physical moves in addition to special moves.
...
#define B_ENCORE_TURNS              GEN_3 // In Gen5+, Encore lasts 3 turns if the target hasn't yet moved this turn, or 4 turns if it has. In Gen4, it lasts 3-7 turns. In Gen2-3, 2-6 turns.
...
#define B_PHYSICAL_SPECIAL_SPLIT    GEN_3 // In Gens1-3, the move's type determines if it will do physical or special damage. The split icon in the summary will reflect this.
...
#define B_MOVE_TUTOR_COSTS          GEN_LATEST // Updates the default move tutor costs of some moves. In general, prices from Gen 6 onwards tend to be a lot cheaper.
#define B_HIDDEN_POWER_COUNTER      GEN_3 // Prior to Gen4, Counter and Mirror Coat treat Hidden Power as Physical regardless of type.
#define B_MODERN_TRICK_CHOICE_LOCK  GEN_3 // In Gen5+, if a Choice Item is swapped for a Choice Item, the Trick/Switcheroo user can pick another move, and then they'll be locked into it.
#define B_PROTECT_FAILURE_RATE      GEN_3 // In Gen5+, protect moves fails 1/3 of the time instead of 1/2
...
#define B_BEAT_UP                   GEN_3 // In Gen5+, Beat Up uses a different formula to calculate its damage, and deals Dark-type damage. Prior to Gen 5, each hit also announces the party member's name.
#define B_DARK_VOID_FAIL            GEN_LATEST // In Gen7+, only Darkrai can use Dark Void.
#define B_HIT_THAW                  GEN_3 // In Gen6+, damaging moves that thaw the user will thaw the target. In Gen 3+, Fire-type moves thaw the target. In Gen 1-2, damaging moves that can burn will thaw the target, regardless if they can be burned or not.
...
#define B_CHARGE                    GEN_3 // In Gen9+, Charge status is lost regardless of the typing of the next move.
#define B_POWDER_STATUS_HEAVY_RAIN  GEN_3 // In Gen7+, Powder doesn't damage the user of a Fire type move in heavy rain.
...
#define B_ENCORE_TARGET             GEN_3 // In Gen5+, encored moves are allowed to choose a target
...
#define B_SYNCHRONIZE_TOXIC         GEN_5 // In Gen5+, if a Pokémon with Synchronize is badly poisoned, the opponent will also become badly poisoned. Previously, the opponent would become regular poisoned.
...
#define B_STURDY                    GEN_5 // In Gen5+, Sturdy causes the Pokémon to have 1 HP remaining if another Pokémon's attack or confusion damage would have brought it from full health to 0 HP.
...
#define B_REDIRECT_ABILITY_IMMUNITY GEN_5 // In Gen5+, Pokémon with Lightning Rod/Storm Drain become immune to Electric/Water-type moves and increase their Sp. Attack by 1 stage on top of the redirecting effect.
...
#define B_STEAL_WILD_ITEMS          GEN_3 // In Gen9+, Thief and Covet steal a wild pokemon's item and send it to the bag. In Gens2-8, the stolen item would be held by the Thief/Covet user.
...
#define B_SMART_WILD_AI_FLAG        FLAG_TEMP_SMART_WILD_AI     // If this flag is set, wild Pokémon will become smart, with all AI flags enabled.
#define B_FLAG_NO_CATCHING          FLAG_TEMP_NO_CATCHING     // If this flag is set, the ability to catch wild Pokémon is disabled.
#define B_FLAG_NO_RUNNING           FLAG_TEMP_NO_RUNNING     // If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
...
#define B_VAR_NO_BAG_USE         VAR_TEMP_NO_BAG_USE     // If 1, the ability to use the bag in battle is disabled in trainer battles. If 2, it is also disabled in wild battles.
...
#define B_SHOW_CATEGORY_ICON                FALSE  // If set to TRUE, it will show an icon in the summary and move relearner showing the move's category.
#define B_HIDE_HEALTHBOX_IN_ANIMS           TRUE  // If set to TRUE, hides healthboxes during move animations.
#define B_WAIT_TIME_MULTIPLIER              6    // This determines how long text pauses in battle last. Vanilla is 16. Lower values result in faster battles.
...
// Catching settings
#define B_SEMI_INVULNERABLE_CATCH       GEN_3 // In Gen4+, you cannot throw a ball against a Pokemon that is in a semi-invulnerable state (dig/fly/etc)
#define B_CATCHING_CHARM_BOOST          20        // % boost in Critical Capture odds if player has the Catching Charm.
#define B_INCAPACITATED_CATCH_BONUS     GEN_3 // In Gen5+, the catch rate bonus for a mon with sleep or freeze is 2.5x. In Gen4 and below its only a 2x bonus.
#define B_LOW_LEVEL_CATCH_BONUS         GEN_3 // In Gen8, a bonus is added to the catch rate if catching a mon lower than level 20. In Gen9, the bonus is only applied to mons lower than level 13.
