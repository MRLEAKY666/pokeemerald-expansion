...
{
    u32 data = 1;
    SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_HP, &data);

    data = (GetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_LEVEL)/2);
    SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_LEVEL, &data);
    u32 species = GetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_SPECIES, NULL);
    SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_EXP, &gExperienceTables[gSpeciesInfo[species].growthRate][data]);
    CalculateMonStats(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004]);

    bool8 dead = FALSE;
    SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_DEAD, &dead);

    data = STATUS1_NONE;
    SetMonData(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004], MON_DATA_STATUS, &data);

    MonRestorePP(&gParties[B_TRAINER_PLAYER][gSpecialVar_0x8004]);
}
...
void Script_SetKO(struct ScriptContext *ctx)
{
    ...
    if (slot < PARTY_SIZE)
    {
        ...
        SetMonData(&gParties[B_TRAINER_PLAYER][slot], MON_DATA_DEAD, &dead);
    }
}
