...
void Script_DoRayquazaScene(void)
{
    if (gSpecialVar_0x8004 == 0)
    {
        ...
        DoRayquazaScene(0, 0, CB2_ReturnToFieldContinueScriptPlayMapMusic);
    }
    else if (gSpecialVar_0x8004 == 1)
    {
        // Rayquaza arrives scene
        DoRayquazaScene(1, 1, CB2_ReturnToFieldContinueScriptPlayMapMusic);
    }
    else if (gSpecialVar_0x8004 == 2)
    {
        // vanilla scene
        DoRayquazaScene(1, 2, CB2_ReturnToFieldContinueScriptPlayMapMusic);
    }
}
...
bool32 CheckPartyHasType(u32 givenType)
{
    ...
    for (partyIndex = 0; partyIndex < CalculatePlayerPartyCount(); partyIndex++)
        if (GetSpeciesType(GetMonData(&gParties[B_TRAINER_PLAYER][partyIndex], MON_DATA_SPECIES), 0) == givenType 
         || GetSpeciesType(GetMonData(&gParties[B_TRAINER_PLAYER][partyIndex], MON_DATA_SPECIES), 1) == givenType)
            return TRUE;
