...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        sStorage->displayMonSpecies = GetMonData(mon, MON_DATA_SPECIES); // changed from MON_DATA_SPECIES_OR_EGG on 21/12/25
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
    ...
    else if (mode == MODE_BOX)
    {
        ...
        sStorage->displayMonSpecies = GetBoxMonData(pokemon, MON_DATA_SPECIES); // changed from MON_DATA_SPECIES_OR_EGG on 26.4.18 as per #9603, check here if custom eggs display wrong in PC
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
