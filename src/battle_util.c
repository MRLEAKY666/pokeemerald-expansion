...
bool32 TryFieldEffects(enum FieldEffectCases caseId)
{
    ...
    case FIELD_EFFECT_OVERWORLD_WEATHER:
        ...
        if (effect)
        {
            //castform changed weather strings don't get pulled up because gBattlerAttacker and gBattlerAbility don't get populated correctly
            //enhancement wiz1989
            ChangeWeather(gBattlerAttacker, gBattlerAbility);
            //enhancement end
        }
        break;
    }
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_WEATHER: // For ability effects that activate when the battle weather changes.
        ...
        case ABILITY_FORECAST:
        {
            ...
            if ((IsBattlerWeatherAffected(GetBattlerHoldEffect(battler), weather, gBattleWeather)
                 || gBattleWeather == B_WEATHER_NONE
                 || !HasWeatherEffect()) // Air Lock active
                 && TryBattleFormChange(battler, FORM_CHANGE_BATTLE_WEATHER, ability)) 
            {
                gBattleScripting.battler = battler;
                //enhancement wiz1989
...
enum Obedience GetAttackerObedienceForAction(void)
{
    ...
    if (B_OBEDIENCE_MECHANICS < GEN_8 && !IsOtherTrainer(gBattleMons[gBattlerAttacker].otId, gBattleMons[gBattlerAttacker].otName))
        return OBEYS;
    if (FlagGet(FLAG_BADGE06_GET)) // Rain Badge, ignore obedience altogether
        return OBEYS;
    ...
    if (FlagGet(FLAG_BADGE05_GET)) // Balance Badge
        obedienceLevel = 35;
    if (FlagGet(FLAG_BADGE08_GET)) // Feather Badge
        obedienceLevel = 45;
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ICE) && IsBattlerWeatherAffected(ctx->holdEffectDef, ctx->weather, B_WEATHER_ICY_ANY) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
bool32 CanMegaEvolve(enum BattlerId battler)
{
    ...
    // Check if Player has a Mega Ring. (COMMENTED OUT TO ALLOW MEGA EVOLUTION WITHOUT MEGA RING)
    /* if (!TESTING
...
//enhancement wiz1989
void ChangeWeather(enum BattlerId battler, u32 ability)
{   
    /* u32 species;
