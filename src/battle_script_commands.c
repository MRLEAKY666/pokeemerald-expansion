...
//enhanced by wiz1989
bool32 CastformTriggerWeatherChange(enum BattlerId battler, enum Move move)
{
...
bool32 IsCastform(enum BattlerId battler)
{
    u32 species;
...
static void Cmd_dofaintanimation(void)
{
    ...
    if (IsOnPlayerSide(battler))
    {
        ...
        {
            bool8 dead = TRUE;
            SetMonData(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battler]], MON_DATA_DEAD, &dead);
        }
    }
    else
    {
