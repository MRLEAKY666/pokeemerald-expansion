...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_MOON_BALL,       {   3,   3,   3,   8,   8,   9,  30,   _,   _,   _, } },
    ...
    { ITEM_MASTER_BALL,     {   _,   _,   _,   4,   4,   4,   4,   7,   9,   9, } },
...
static void Cmd_attackcanceler(void)
{
    ...
    if (CastformTriggerWeatherChange(gBattlerAttacker, gBattlerAbility, gCurrentMove))
    {
        ...
        /* DoSoftReset(); */ //was debugging here
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
        ...
        // below deviates from expansion starting with Commit 1c1a98e (added scripting code lines to 'if' and added 'else' around same lines, expansion has no 'else')
        ...
        // above deviates from expansion starting with Commit 1c1a98e (added scripting code lines to 'if' and added 'else' around same lines, expansion has no 'else')
...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    else
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
            moneyReward = 2 * lastMonLevel * gBattleStruct->moneyMultiplier * trainerMoney;
        else if (IsDoubleBattle())
            moneyReward = 2 * lastMonLevel * gBattleStruct->moneyMultiplier * 2 * trainerMoney;
        else
            moneyReward = 2 * lastMonLevel * gBattleStruct->moneyMultiplier * trainerMoney;
    }
