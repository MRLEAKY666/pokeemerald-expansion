...
static const u16 sSizeScreenSilhouette_inverted_Pal[] = INCGFX_U16("graphics/pokedex/size_silhouette_inverted.pal", ".gbapal");
...
static const u8 sText_EVO_LEVEL_SILCOON[] = _("{LV}{UP_ARROW} to {STR_VAR_2}, Outgoing");
static const u8 sText_EVO_LEVEL_CASCOON[] = _("{LV}{UP_ARROW} to {STR_VAR_2}, Shy");
...
static void PrintStatsScreen_Moves_Top(u8 taskId)
{
    ...
    if (gTasks[taskId].data[5] == 0)
    {
        SetTypeIconPosAndPal(GetMoveType(move), moves_x + 145, moves_y + 17, 0);
        SetSpriteInvisibility(1, TRUE);
    }
    else
    {
        SetTypeIconPosAndPal(NUMBER_OF_MON_TYPES + GetMoveContestCategory(move), moves_x + 145, moves_y + 17, 1);
        SetSpriteInvisibility(0, TRUE);
    }
