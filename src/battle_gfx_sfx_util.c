...
{
    u32 personalityValue, paletteOffset, targetSpecies, otId, ivs;
    enum BattlerPosition position;
    bool32 isShiny;
    const void *src;
    const u16 *paletteData;
    ...
    else
    {
        ...
        if (changeType == SPECIES_GFX_CHANGE_TRANSFORM)
        {
            ...
            otId = GetMonData(&gParties[B_TRAINER_PLAYER][gBattlerPartyIndexes[battlerAtk]], MON_DATA_OT_ID); */ 
            //gBattleSpritesDataPtr->battlerData[battlerAtk].transformSpecies = targetSpecies; 
        }
        else
        {
            targetSpecies = GetMonData(monAtk, MON_DATA_SPECIES);
            /* if (B_TRANSFORM_SHINY >= GEN_4 && trackEnemyPersonality && !megaEvo) // commented below to resolve merge conflicts from # 8146 to fix transform
            {
            ...
            else
            {
                ...
                isShiny = GetMonData(monAtk, MON_DATA_IS_SHINY);
                otId = GetMonData(&gParties[B_TRAINER_OPPONENT_A][gBattlerPartyIndexes[battlerAtk]], MON_DATA_OT_ID);
            } */
        }
        gBattleSpritesDataPtr->battlerData[battlerAtk].transformSpecies = targetSpecies;
    ...
    if (IsContest())
    {
        ...
        CreateBoxMonLegacy(&boxMon, targetSpecies, 5, USE_RANDOM_IVS, TRUE, personalityValue, OT_ID_PRESET, otId);
