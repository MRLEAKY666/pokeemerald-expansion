...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        ...
        return 0xFF;
    case 1:
        pal = GetMonSpritePalFromSpeciesAndPersonalityIsEgg(summary->species, summary->isShiny, summary->pid, summary->isEgg);
        LoadUncompressedUniqueSpritePalette(pal, &mon->box); // pal definition above was changed on 21/12/25 so chekc here for manaphy egg or egg general weird palettes
